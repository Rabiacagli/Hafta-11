...
public class CategoryController {
   ...
   /* @PostMapping()  // mapping bölümü belli  oldugu için içi boş  bırakılabılır
    @ResponseStatus(HttpStatus.CREATED)
    // yeni bir nesne kaydederken bunu Category'e cevırmemız gerekli
    // request'ı category'e , category'ı de response'a cevırıyoruz
   ...
   /* @PostMapping()
    @ResponseStatus(HttpStatus.CREATED)
    public Result save (@Valid @RequestBody CategorySaveRequest categorySaveRequest) {
        Category saveCategory = this.modelMapper.forRequest().map(categorySaveRequest, Category.class);
        this.categoryService.save(saveCategory);
        return new Result(true,"Veri eklendi", "201");
    }*/

    @PostMapping()   // save işlemi son hali
    @ResponseStatus(HttpStatus.CREATED)
    ...
    @GetMapping("/{id}")  // id'ye gore veri cekme ıslemı
    ...
    @PutMapping()   // update işlemi   --- dto içerisinde once updateRequest oluşturuyoruz
    @ResponseStatus(HttpStatus.OK)
    ...
    @DeleteMapping("/{id}")   // delete işlemi
    @ResponseStatus(HttpStatus.OK)
    ...
    // sayfa sayısına gore veri dondurme ıslemı
 /*   @GetMapping()
    @ResponseStatus(HttpStatus.OK)
